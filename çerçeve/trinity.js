...
module.exports = class EchoCommand extends commando.Command {
    constructor(client) {
        super(client, {
            ...
            group: 'çerçeve',
            memberName: 'trinity',
            description: 'Profil fotoğrafınıza Balance efekti verir.',
            guildOnly: true,
			throttling: {
				usages: 2,
				duration: 6
			...
			args: [
				{
					key: 'member',
					label: 'user',
					prompt: 'Kimin profil fotoğrafına trinity efekti yapmak istiyorsun?\n',
					type: 'member'
				}
			]
        });
    ...
    async run(msg, args) {
      ...
      if (voted) {
        ...
        msg.channel.send(`:mag_right: **Trinity** efekti uygulanıyor, lütfen bekleyiniz.`).then(m => m.delete(3000));

        const embed = new Discord.RichEmbed()
        .setImage('https://eggsy.codes/api/overlay/create?overlay=trinity&url=' + user.avatarURL.toString().replace('2048', '1024&width=256&height=256'))
