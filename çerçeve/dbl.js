...
module.exports = class EchoCommand extends commando.Command {
    constructor(client) {
        super(client, {
            ...
            group: 'çerçeve',
            memberName: 'dbl',
            description: 'Profil fotoğrafınıza Dbl efekti verir.',
            guildOnly: true,
			throttling: {
				usages: 2,
				duration: 6
			...
			args: [
				{
					key: 'member',
					label: 'user',
					prompt: 'Kimin profil fotoğrafına dbl fekti yapmak istiyorsun?\n',
					type: 'member'
				}
			]
        });
    ...
    async run(msg, args) {
      ...
      if (voted) {
        ...
        msg.channel.send(`:mag_right: **Dbl** efekti uygulanıyor, lütfen bekleyiniz.`).then(m => m.delete(3000));

        const embed = new Discord.RichEmbed()
        .setImage('https://eggsy.codes/api/overlay/create?overlay=dbl&url=' + user.avatarURL.toString().replace('2048', '1024&width=256&height=256'))
