...
module.exports = class EchoCommand extends commando.Command {
			...
			args: [
				{
					key: 'member',
					label: 'user',
					prompt: 'Kimin profil fotoğrafına Hypesquad Balance efekti yapmak istiyorsun?\n',
					type: 'member'
				}
			]
        });
    ...
    async run(msg, args) {
      ...
      if (voted) {
        ...
        msg.channel.send(`:mag_right: **Balance** efekti uygulanıyor, lütfen bekleyiniz.`).then(m => m.delete(3000));

        Jimp.read(user.avatarURL, (err, image) => {
            image.resize(310, 325)
            image.greyscale()
            image.gaussian(3)
            Jimp.read("https://cdn.glitch.com/e7067fac-0d60-4c5a-a9d2-eeea3c83cbf4%2Fbala.png?v=1574287627660", (err, avatar) => {
