...
module.exports = class EchoCommand extends commando.Command {
    constructor(client) {
        super(client, {
            ...
            group: 'çerçeve',
            memberName: 'ulcode',
            description: 'Profil fotoğrafınıza ulcode efekti verir.',
            guildOnly: true,
			throttling: {
				usages: 2,
				duration: 6
			...
			args: [
				{
					key: 'member',
					label: 'user',
					prompt: 'Kimin profil fotoğrafına ulcode efekti yapmak istiyorsun?\n',
					type: 'member'
				}
			]
        });
    ...
    async run(msg, args) {
      ...
      if (voted) {
        ...
        msg.channel.send(`:mag_right: **Ulcode** efekti uygulanıyor, lütfen bekleyiniz.`).then(m => m.delete(3000));

        const embed = new Discord.RichEmbed()
        .setImage('https://eggsy.codes/api/overlay/create?overlay=ulcode&url=' + user.avatarURL.toString().replace('2048', '1024&width=256&height=256'))
